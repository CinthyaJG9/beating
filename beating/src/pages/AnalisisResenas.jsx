...
const AnalisisResenas = () => {
  ...
  const totalResenas = data.distribucion_sentimientos.reduce((acc, curr) => acc + curr.cantidad, 0);
  const mejorPuntuacion = Math.max(...data.mejores_canciones.map(item => item.puntuacion));
  const artistaMasResenado = data.mejores_canciones.reduce((prev, current) => 
    (prev.reseñas > current.reseñas) ? prev : current
  ).artista;

  return (
    <div className="min-h-screen bg-[#1e1626] text-white">
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div className="flex overflow-x-auto border-b border-white/10 mb-8">
          {['general', 'canciones', 'sentimientos'].map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`px-6 py-3 font-medium transition-colors ${activeTab === tab ? 'text-pink-400 border-b-2 border-pink-400' : 'text-gray-400 hover:text-white'}`}
            >
              {tab === 'general' ? 'Visión General' : tab === 'canciones' ? 'Top Canciones' : 'Distribución'}
            </button>
          ))}
        </div>
          ...
          {(activeTab === 'general' || activeTab === 'canciones') && (
            <div className={cardStyle}>
              ...
              <div className="bg-black/20 rounded-xl p-4 mb-4 border border-purple-500/30">
                <img src={`data:image/png;base64,${data.top_songs}`} alt="Top canciones" className="w-full h-auto rounded" />
              </div>
          ...
          {(activeTab === 'general' || activeTab === 'sentimientos') && (
            <div className={cardStyle}>
              ...
              <div className="bg-black/20 rounded-xl p-4 mb-6 border border-purple-500/30">
                <img src={`data:image/png;base64,${data.sentiment_dist}`} alt="Distribución sentimientos" className="w-full h-auto rounded" />
              </div>
          ...
          {activeTab === 'general' && (
            <div className={`${cardStyle} lg:col-span-2`}>
              ...
              <div className="bg-black/20 rounded-xl p-4 border border-purple-500/30">
                <img src={`data:image/png;base64,${data.wordcloud}`} alt="Nube de palabras" className="w-full h-auto rounded" />
              </div>
            </div>
          )}
        </div>
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className={`${cardStyle}`}>
            ...
            <p className="text-2xl font-bold text-white truncate text-center pt-2">{artistaMasResenado}</p>
          </div>
        </div>
      </div>
    </div>
