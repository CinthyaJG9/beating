...
const Resenas = () => {
  ...
  const [searchType, setSearchType] = useState('artist'); // 'artist' o 'album'
  ...
  // Buscar álbumes directamente
  const searchAlbums = async () => {
    if (!searchTerm.trim()) {
      setMessage('Por favor ingresa un nombre de álbum o artista');
      return;
    }
    ...
    try {
      const response = await axios.get(`http://localhost:5000/api/albumes/buscar?q=${encodeURIComponent(searchTerm)}`, {
        timeout: 10000
      });
      
      console.log('✅ Respuesta de álbumes:', response.data);
      
      setAlbums(response.data || []);
      setTotalPages(Math.ceil(response.data.length / itemsPerPage));
      setMessage(response.data?.length ? '' : 'No se encontraron álbumes');
      setActiveTab('albumResults');
    } catch (error) {
      ...
      if (error.code === 'ECONNABORTED') {
        ...
        setMessage('Error al buscar álbumes: ' + (error.response?.data?.error || error.message));
      }
    } finally {
      setLoading(false);
    }
  ...
  // Seleccionar álbum de búsqueda directa
  const handleSelectAlbum = (album) => {
    const simulatedAlbum = {
      id: album.id_spotify || album.id_album,
      name: album.titulo,
      artist: album.artista,
      image: album.imagen_url,
      year: album.fecha_lanzamiento ? album.fecha_lanzamiento.substring(0, 4) : 'Desconocido'
    };
...
const crearPlaylistSpotify = async () => {
    try {
        ...
        if (!response.ok) {
            // 🔥 MANEJO MEJORADO DE ERROR 401 - REDIRECCIÓN INMEDIATA
            if (response.status === 401 && data.auth_required && data.auth_url) {
                console.log("🔐 Redirigiendo a autenticación de Spotify...");
        ...
        // Si llegamos aquí, la playlist se creó exitosamente
      ...
      <div className="container mx-auto px-4 py-6 max-w-6xl">
        ...
        <div className="mb-8 bg-white/5 rounded-2xl backdrop-blur-sm border border-white/10 overflow-hidden">
          <Tabs>
            ...
            <Tab 
              isActive={activeTab === 'albumResults' && albums.length > 0} 
              onClick={() => albums.length > 0 && setActiveTab('albumResults')}
        ...
        <div className="bg-white/5 rounded-2xl backdrop-blur-sm border border-white/10 p-8">
          ...
          {activeTab === 'search' && (
            <div className="space-y-6">
              <h2 className="text-3xl font-bold text-white mb-4">Buscar</h2>
              ...
              <div className="flex flex-col md:flex-row gap-4">
                <input
                  ...
                  placeholder={
                    searchType === 'artist' 
                      ? "Busca un artista (ej. The Beatles, ABBA)..."
                      : "Busca un álbum (ej. Abbey Road, Thriller)..."
                  }
                ...
                >
                  {loading ? (
                    ...
                    `Buscar ${searchType === 'artist' ? 'Artista' : 'Álbum'}`
                  )}
                </button>
              </div>
            </div>
          ...
          {/* Pestaña de resultados de álbumes */}
          {activeTab === 'albumResults' && albums.length > 0 && (
            <div className="space-y-6">
              <h2 className="text-3xl font-bold text-white mb-6">Álbumes Encontrados</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {currentAlbums.map(album => (
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      <h4 className="font-semibold text-lg text-white truncate">{album.titulo}</h4>
                      <p className="text-blue-300 text-sm">{album.artista}</p>
                      {album.fecha_lanzamiento && (
                        <p className="text-gray-400 text-xs">📅 {album.fecha_lanzamiento.substring(0, 4)}</p>
                      )}
                      {album.total_canciones && (
                        <p className="text-gray-400 text-xs">🎵 {album.total_canciones} canciones</p>
                      )}
                      {album.existe_en_bd && (
                        <p className="text-green-400 text-xs">✅ En nuestra base de datos</p>
                      )}
                    </div>
                  </div>
                ))}
